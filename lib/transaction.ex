defmodule BitcoinLib.Transaction do
  ...
  alias BitcoinLib.Transaction.{Input, Encoder, Decoder, BetterSigner}
  ...
  """
  @spec sign_and_encode(Transaction.t(), list(PrivateKey.t())) :: binary()
  def sign_and_encode(%Transaction{} = transaction, private_keys) do
    with {:ok, signed_transaction} <- BetterSigner.sign(transaction, private_keys) do
      {
        :ok,
        signed_transaction |> Transaction.encode() |> Binary.to_hex()
      }
    else
      {:error, message} -> {:error, message}
    end
  ...
  @strip_signatures_opts_defaults %{serialization_format: nil}
  @doc """
  Signs a transaction and transforms it into a binary that can be sent to the network

  ## Examples
      iex>  input_keeping_sig = %BitcoinLib.Transaction.Input{
      ...>    txid: "e4c226432a9319d603b2ed1fa609bffe4cd91f89b3176a9e73b19f7891a92bb6",
      ...>    vout: 0,
      ...>    sequence: 0xFFFFFFFF,
      ...>    script_sig: <<0x76A914AFC3E518577316386188AF748A816CD14CE333F288AC::200>> |> BitcoinLib.Script.parse!()
      ...>  }
      ...>  input_to_strip = %BitcoinLib.Transaction.Input{
      ...>    txid: "3f4fa19803dec4d6a84fae3821da7ac7577080ef75451294e71f9b20e0ab1e7b",
      ...>    vout: 0,
      ...>    script_sig: <<0x76A914AFC3E518577316386188AF748A816CD14CE333F288AC::200>> |> BitcoinLib.Script.parse!(),
      ...>    sequence: 0xFFFFFFFF
      ...
      ...>    version: 1,
      ...>    inputs: [
      ...>      input_keeping_sig, input_to_strip
      ...>    ],
      ...>    outputs: [%BitcoinLib.Transaction.Output{
      ...>       script_pub_key: <<0x76a9283265393261373463333431393661303236653839653061643561633431386366393430613361663288ac::360>> |> BitcoinLib.Script.parse!(),
      ...>       value: 10_000}
      ...
      ...>    locktime: 0
      ...>  }
      ...>  |> BitcoinLib.Transaction.strip_signatures(except: input_keeping_sig)
      %BitcoinLib.Transaction{
        version: 1,
        inputs: [
          %BitcoinLib.Transaction.Input{
  ...
  """
  def strip_signatures(%Transaction{inputs: inputs} = transaction, opts) do
    %{except: exception} = Enum.into(opts, @strip_signatures_opts_defaults)
