defmodule BitcoinLib.Base58Check do
  @moduledoc """
  ...
  def base_encode(<<0>> <> payload) when byte_size(payload) > 0,
    do: base_encode(<<0>>) <> base_encode(payload)
  ...
  def base_decode!(<<@code_0>> <> string) when byte_size(string) > 0,
    do: base_decode!(<<@code_0>>) <> base_decode!(string)
  ...
  def checksum(payload), do: payload |> Crypto.double_sha256() |> Binary.take(4)
end
